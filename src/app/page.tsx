...
import { listHeroSlides, listSchedule, getFeaturedNews } from "@/lib/db"; // 匯入從 MongoDB 讀取首頁焦點、賽程、精選報導的函式
...
export default async function Home() { // 匯出首頁元件(對應路由 "/")
  const [heroSlides, schedule, featuredArticle] = await Promise.all([ // 平行抓取首頁焦點、賽程、精選報導資料
    listHeroSlides(),
    listSchedule(),
    getFeaturedNews(),
  ]);
  ...
  return ( // 回傳首頁的畫面結構
    <> {/* 使用 Fragment 包裹多個區塊，不會產生多餘的 DOM 節點 */}
      ...
      <FeaturedSpread article={featuredArticle} /> {/* 渲染精選內容區塊(後台沒有指定精選文章時不顯示) */}
    </>
  );
}

...
import { listPortfolioAlbums } from "@/lib/db"; // 匯入從 MongoDB 讀取作品集相簿的函式

export const metadata: Metadata = { // 匯出這個頁面的中繼資料設定
  title: "作品集", // 網頁標題
  description: "每一本相簿都是一場比賽的完整記錄——起跑、交棒、衝線、頒獎。", // 網頁描述文字
}; // metadata 設定結束
...
export default async function PortfolioPage() { // 匯出作品集頁面元件（Next.js App Router 的頁面進入點）
  const portfolio = await listPortfolioAlbums(); // 從資料庫抓取所有相簿
